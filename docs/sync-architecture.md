...
测试覆盖原始 SQL 事务、RETURNING 中断、触发器副作用、日志失败、进程崩溃、固定批次合并、部分成功、下载检查点失败、账号变化及驱动升级。CloudKit 网络使用 fixture，系统调度和真实容器权限需要宿主真机验证。

## 实体同步范围

`@Entity(sync: false)` 生成 `isSyncEnabled = false` 元数据。数据库 schema 和业务 migration 使用全部注册实体；变更跟踪、首次快照和远端数据应用只使用参与同步的实体。

SyncManager 保留明确标记为本地的实体名集合。上传批次保留这些历史日志的序号，但不为它们生成上传项；确认时把它们视为可跳过事件，仍按连续前缀推进游标，不能跨过未确认的同步事件。纯本地批次在本地事务中提交游标后继续扫描；提交失败不推进游标，重试重新读取。原始日志保持不可修改。

下载页面先排除本地实体，再校验和应用其余记录，最后在同一事务中保存检查点。仅排除明确标记为本地的实体，未知实体及同步实体的无效数据仍阻止页面提交。

同步范围在 manager 初始化时确定；不支持运行时切换或自动清理已有云端记录。

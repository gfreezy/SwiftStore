...
/// ManyToOne relation macro
public struct ManyToOneMacro: MemberMacro, PeerMacro {
    public static func expansion(
        ...
        guard let structDecl = declaration.as(StructDeclSyntax.self) else {
            throw MacroError.message("@ManyToOne can only be applied to structs")
        }

        guard let arguments = node.arguments?.as(LabeledExprListSyntax.self) else {
            throw MacroError.message("@ManyToOne requires arguments")
        }

        let config = try RelationConfig.parse(from: arguments, relationType: .manyToOne)
        ...
        if config.hasFrom {
            guard let fromKey = config.fromKey else {
                throw MacroError.message("@ManyToOne requires 'fromKey' parameter")
            }
            guard let toKey = config.toKey else {
                throw MacroError.message("@ManyToOne requires 'toKey' parameter")
            }
        ...
        guard let fromField = config.fromField else {
            throw MacroError.message("@ManyToOne requires 'fromField' parameter")
        }
    ...
    private static func generateRelationMarkerMembers(config: RelationConfig) throws -> [DeclSyntax] {
        guard let fromType = config.fromType,
            ...
            throw MacroError.message("@ManyToOne with 'from' requires fromType, fromKey, and toKey")
        }
        ...
        let relationTypeDecl: DeclSyntax = """
            public static var relationType: RelationType { .manyToOne }
            """

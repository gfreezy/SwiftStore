...
// MARK: - Store extensions for SQL type

public extension Store {
    /// Execute type-safe SQL (uses embedded values)
    @discardableResult
    func execute(_ sql: SQL) throws -> Int {
        try execute(sql.sql, values: sql.values)
    }

    /// Execute type-safe SQL with additional parameters
    @discardableResult
    func execute(_ sql: SQL, values: [SQLiteValue]) throws -> Int {
        try execute(sql.sql, values: sql.values + values)
    }

    /// Query returning Row objects using type-safe SQL
    func query(_ sql: SQL) throws -> [Row] {
        try query(sql.sql, values: sql.values)
    }

    /// Query single Row using type-safe SQL
    func queryOne(_ sql: SQL) throws -> Row? {
        try queryOne(sql.sql, values: sql.values)
    }

    /// Query scalar using type-safe SQL (uses embedded values)
    func queryScalar<T>(_ sql: SQL) throws -> T? {

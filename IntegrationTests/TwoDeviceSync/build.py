...
objects = [p for m in modules for p in (build / (m + '.build')).glob('*.swift.o')]
...
subprocess.run(['xcrun', 'swiftc', '-parse-as-library', '-target', 'arm64-apple-ios16.0-simulator',
                ...
                str(Path(__file__).with_name('Worker.swift')), *map(str, objects), '-lsqlite3', '-o', str(OUTPUT)], check=True)

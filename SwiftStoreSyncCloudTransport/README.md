...
用 `@Entity(sync: false)` 声明缓存、设备设置等本地实体，仍将它们注册到 `ConnectionManager.entities`。默认可写实体参与同步；只读实体不参与同步。此参数不改变表结构、业务 migration、CRUD、事务或可写实体的字段要求。

本地实体不生成 changelog、不导出首次同步快照。上传遇到已有本地实体日志时直接跳过；连续上传游标不能越过未确认的同步日志。整批都是本地日志时，在本地提交游标后继续读下一批，不创建空网络上传。下载遇到明确注册为本地的实体时忽略该记录并正常提交页面游标，未知实体仍报错。

该配置在 manager 生命周期内固定，不作为运行时开关，也不自动删除云端已有记录。以后重新开启某实体的同步，需要单独设计已有数据的重新同步策略。
